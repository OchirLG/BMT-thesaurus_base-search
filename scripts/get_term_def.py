...
def get_med_definition(term: str, context: str, max_retries: int = 2, suggest: str = None) -> dict:
    ...
    ### 1. ПРАВИЛО "NULL" ПРИ НЕУВЕРЕННОСТИ:
    Если термин — аббревиатура, и она:
    А) Не расшифрована явно в предоставленном тексте.
    Б) Ты не можешь со 100% уверенностью определить её значение из контекста (якорей домена).
    ТОГДА: установи значения "expansion": null и "definition": null.

    ### 2. СТРУКТУРА JSON (СТРОГО):
    {
    "reasoning": "Пошаговый разбор: поиск словарей-якорей, оценка уверенности.",
